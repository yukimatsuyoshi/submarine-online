...
function drawRanking(ctxScore, playersMap) {
  const playersArray = [].concat(Array.from(playersMap)); // Arrayをsortした時に，Mapまでsortされないようにする
...
function drawMap(gameObj) {
  ...
  // 敵プレイヤーとNPCの描画
  for (let [key, enemyObj] of gameObj.playersMap) {
    if (key === gameObj.myPlayerObj.playerId) { continue; } // 自分は対象外

    const distanceObj = calculationBetweenTwoPoints(
      gameObj.myPlayerObj.x, gameObj.myPlayerObj.y,
      enemyObj.x, enemyObj.y,

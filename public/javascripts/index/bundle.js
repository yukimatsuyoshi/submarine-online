...
function drawSubmarine(ctxRader, myPlayerObj) {
  ...
  if (myPlayerObj.isAlive === false) {
    drawBom(ctxRader, gameObj.raderCanvasWidth / 2, gameObj.scoreCanvasHeight / 2, myPlayerObj.deadCount);
    return;
  }
...
function drawBom(ctxRader, drawX, drawY, deadCount) {
  ...
  ctxRader.drawImage(gameObj.bomListImage, //  画像データ
  cropX, cropY, //　切り抜き左、切り抜き上
  gameObj.bomCellPx, gameObj.bomCellPx, // 切り抜き幅    
  drawX - gameObj.bomCellPx / 2, drawY - gameObj.bomCellPx / 2, // 表示x、表示y
  gameObj.bomCellPx, gameObj.bomCellPx // 表示幅
  );
}
...
function drawMap(gameObj) {
  ...
  try {
    for (var _iterator = gameObj.playersMap[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      var key = _ref2[0];
      var enemyObj = _ref2[1];

      if (key === gameObj.myPlayerObj.playerId) {
        continue;
      } // 自分は対象外

      var distanceObj = calculationBetweenTwoPoints(gameObj.myPlayerObj.x, gameObj.myPlayerObj.y, enemyObj.x, enemyObj.y, gameObj.fieldWidth, gameObj.fieldHeight, gameObj.raderCanvasWidth, gameObj.raderCanvasHeight);

      if (distanceObj.distanceX <= gameObj.raderCanvasWidth / 2 && distanceObj.distanceY <= gameObj.raderCanvasHeight / 2) {

        if (enemyObj.isAlive === false) {
          drawBom(gameObj.ctxRader, distanceObj.drawX, distanceObj.drawY, enemyObj.deadCount);
          continue;
        }
        ...
        if (enemyObj.displayName === 'anonymous') {
          ...
          gameObj.ctxRader.font = '8px Arial';
          gameObj.ctxRader.fillText(enemyObj.displayName, distanceObj.drawX + 20, distanceObj.drawY - 20 - 1);
        }
      }
    }
